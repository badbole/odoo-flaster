...
class BaseExternalDbsource(models.Model):
    ...
    def _execute_sqlalchemy(self, sqlquery, sqlparams, metadata):
        ...
        for record in self:
            with record.connection_open() as connection:
                # DB: fix based on: https://techoverflow.net/2024/07/06/how-to-fix-sqlalchemy-exc-objectnotexecutableerror-not-an-executable-object/
                if sqlparams is None:
                    # cur = connection.execute(sqlquery)
                    cur = connection.execute(sqlalchemy.sql.text(sqlquery))
                else:
                    cur = connection.execute(sqlalchemy.sql.text(sqlquery), sqlparams)
                if metadata:
                    cols = list(cur.keys())
    ...
    def _execute_sqlalchemy_json(self, sqlquery, sqlparams, metadata):
        rows = list()
        for record in self:
            with record.connection_open() as connection:
                if sqlparams is None:
